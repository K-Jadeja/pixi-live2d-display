...
function initializeTTSSystem() {
  ...
  streamingAudioPlayer = new StreamingLive2DAudioPlayer(ttsWorker, model);
  ...
  const onMessageReceived = async (e) => {
    switch (e.data.status) {
      ...
      case "stream_audio_data":
        ...
        // Queue audio data in our streaming Live2D audio player
        await streamingAudioPlayer.queueAudio(e.data.audio);
        break;

      case "complete":
        try {
          updateProgress(95, "Finalizing streaming audio...");

          // Finalize the streaming audio
          await streamingAudioPlayer.finalizeAudio();
...
async function loadModel(modelPath, modelName) {
  try {
    ...
    // Update streaming audio player with new model
