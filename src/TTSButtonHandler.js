...
export class TTSButtonHandler {
  constructor(worker, streamingAudioPlayer) {
    this.worker = worker;
    this.streamingAudioPlayer = streamingAudioPlayer;
  ...
  async handleSpeakButtonClick() {
    ...
    if (!this.streamingAudioPlayer.live2dModel) {
    ...
    try {
      ...
      this.streamingAudioPlayer.setTotalChunks(Math.ceil(text.length / 300));
      this.streamingAudioPlayer.reset();
